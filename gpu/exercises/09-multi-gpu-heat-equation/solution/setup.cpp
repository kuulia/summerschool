...
void initialize(int argc, char *argv[], field *current,
                ...
                field *gpu_current, field *gpu_previous)
{
    /*
    ...
    *gpu_current = *current;
    *gpu_previous = *previous;

    size_t num_bytes = sizeof(double)*(current->nx+2)*(current->ny+2);
    hipMalloc(&(gpu_current->data), num_bytes);
    hipMalloc(&(gpu_previous->data), num_bytes);
    hipMemcpy(gpu_current->data, current->data, num_bytes, hipMemcpyDefault);
    hipMemcpy(gpu_previous->data, previous->data, num_bytes, hipMemcpyDefault);
}
