...
struct GPUBuffer {
    ...
    explicit GPUBuffer(size_t n_) : n(n_) {
        HIP_ERRCHK(hipMalloc(&ptr, n * sizeof(T)));
    }
    ~GPUBuffer() {
        if (ptr) HIP_ERRCHK(hipFree(ptr));
    }
    ...
    void upload(const T* host) {
        HIP_ERRCHK(hipMemcpy(ptr, host, n * sizeof(T), hipMemcpyHostToDevice));
    }
    void download(T* host) {
        HIP_ERRCHK(hipMemcpy(host, ptr, n * sizeof(T), hipMemcpyDeviceToHost));
    }
...
def gen_launch(line, buffer_info, explicit_mem):
    ...
    return (
        ...
        f"HIP_ERRCHK(hipGetLastError());\n"
        f"HIP_ERRCHK(hipDeviceSynchronize());"
    )
