 ...
 *   - perform recurring allocations in a loop using
...
void noRecurringAlloc(int nSteps, int size) {
  ...
  int *d_A;
  // Allocate device memory (allocate memory with hipMalloc for d_A of size
  // `bytes`)
  HIP_ERRCHK(hipMalloc((void **)&d_A, bytes));
  ...
  // Free allocation free d_A allocation using hipFree
  HIP_ERRCHK(hipFree(d_A));
}
...
void recurringAllocMallocAsync(int nSteps, int size) {
  ...
  for (unsigned int i = 0; i < nSteps; i++) {
    int *d_A;
    // Allocate device memory, allocate memory with hipMallocAsync for d_A of
    // size `bytes` in stream
    HIP_ERRCHK(hipMallocAsync((void **)&d_A, bytes, stream));
    // Launch GPU kernel
    hipKernel<<<gridsize, blocksize, 0, stream>>>(d_A, size);
    HIP_ERRCHK(hipGetLastError());
    // Free allocation free d_A allocation using hipFreeAsync in stream
