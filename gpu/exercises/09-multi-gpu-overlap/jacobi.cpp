...
// GPU kernel
__global__ void sweepGPU(double *phi, const double *phiPrev,
                         const double *source, double h2, int Nrows,
                         int Ncols) {
  ...
  if (i > 0 && j > 0 && i < Ncols - 1 && j < Nrows - 1)
    phi[index] = 0.25 * (phiPrev[i1] + phiPrev[i2] + phiPrev[i3] + phiPrev[i4] -
                         h2 * source[index]);
}

// GPU kernel
__global__ void sweepGPU_boundary(double *phi, const double *phiPrev,
                                  const double *source, double h2, int Nrows,
                                  int Ncols) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  // First row
...
int main(int argc, char **argv) {
  ...
  if (0 == rank)
    nghbrs[0] = MPI_PROC_NULL;
  if (ntasks - 1 == rank)
    nghbrs[1] = MPI_PROC_NULL;
  ...
  if (COMPUTE_CPU_REFERENCE) {
    ...
    while (diff > tolerance && iterations < MAX_ITERATIONS) {
      ...
      MPI_Sendrecv(&phi[(Nrows - 2) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1], 0,
                   &phi[0], Ncols, MPI_DOUBLE, nghbrs[0], 0, MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE);
      sweepCPU(phiPrev, phi, source, h * h, Nrows, Ncols);

      // Halo exchange
      MPI_Sendrecv(&phiPrev[Ncols], Ncols, MPI_DOUBLE, nghbrs[0], 0,
                   &phiPrev[(Nrows - 1) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1],
                   0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Sendrecv(&phiPrev[(Nrows - 2) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1],
                   0, &phiPrev[0], Ncols, MPI_DOUBLE, nghbrs[0], 0,
                   MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  ...
  dim3 dimGrid((Ncols + blocksize - 1) / blocksize,
               (Nrows + blocksize - 1) / blocksize);
  ...
  while (diff > tolerance && iterations < MAX_ITERATIONS) {
    ...
    MPI_Sendrecv(&phi_d[(Nrows - 2) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1], 0,
                 &phi_d[0], Ncols, MPI_DOUBLE, nghbrs[0], 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
    roctxRangePop();
    sweepGPU<<<dimGrid, dimBlock>>>(phiPrev_d, phi_d, source_d, h * h, Nrows,
                                    Ncols);
    HIP_CHECK(hipDeviceSynchronize());

    roctxRangePush("halo_exchange");
    MPI_Sendrecv(&phiPrev_d[Ncols], Ncols, MPI_DOUBLE, nghbrs[0], 0,
                 &phiPrev_d[(Nrows - 1) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1],
                 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Sendrecv(&phiPrev_d[(Nrows - 2) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1],
                 0, &phiPrev_d[0], Ncols, MPI_DOUBLE, nghbrs[0], 0,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    roctxRangePop();
    sweepGPU<<<dimGrid, dimBlock>>>(phi_d, phiPrev_d, source_d, h * h, Nrows,
                                    Ncols);
    HIP_CHECK(hipDeviceSynchronize());
