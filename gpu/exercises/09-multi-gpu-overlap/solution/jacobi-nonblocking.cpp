...
{ 
    ...
    dim3 dimGrid_b((Ncols + blocksize*blocksize - 1) / blocksize*blocksize);
    ...
    while (diff > tolerance && iterations < MAX_ITERATIONS) {
        ...
        sweepGPU_interior<<<dimGrid, dimBlock>>>(phiPrev_d, phi_d, source_d, h*h, Nrows, Ncols); 

        roctxRangePush("halo_exchange_init");
        ...
        roctxRangePush("halo_exchange_finalize");
        ...
        sweepGPU_interior<<<dimGrid, dimBlock>>>(phi_d, phiPrev_d, source_d, h*h, Nrows, Ncols); 

        roctxRangePush("halo_exchange_init");
        MPI_Irecv(&phiPrev_d[(Nrows-1) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1], 0, MPI_COMM_WORLD, &reqs[0]);
        MPI_Irecv(&phiPrev_d[0], Ncols, MPI_DOUBLE, nghbrs[0], 0, MPI_COMM_WORLD, &reqs[1]);
        MPI_Isend(&phiPrev_d[Ncols], Ncols, MPI_DOUBLE, nghbrs[0], 0, MPI_COMM_WORLD, &reqs[2]);
        MPI_Isend(&phiPrev_d[(Nrows-2) * Ncols], Ncols, MPI_DOUBLE, nghbrs[1], 0, MPI_COMM_WORLD, &reqs[3]);
        roctxRangePop();
        ...
        sweepGPU_boundary<<<dimGrid_b, dimBlock_b>>>(phi_d, phiPrev_d, source_d, h*h, Nrows, Ncols); 
        HIP_CHECK( hipDeviceSynchronize() );
