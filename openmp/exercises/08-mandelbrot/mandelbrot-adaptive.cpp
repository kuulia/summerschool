...
void compute_block(iter_t *iter_counts, index_t width, index_t i0, index_t j0,
                   index_t i1, index_t j1, double xmin, double ymin, double dx,
                   double dy) {
...
void fill_block(iter_t *iter_counts, index_t width, index_t i0, index_t j0,
                index_t i1, index_t j1, iter_t value) {
  for (index_t j = j0; j < j1; ++j) {
    for (index_t i = i0; i < i1; ++i) {
...
void compute_adaptive(iter_t *iter_counts, index_t width, index_t i0,
                      index_t j0, index_t i1, index_t j1, double xmin,
                      double ymin, double dx, double dy) {

  // Compute small block directly
  index_t w = i1 - i0;
  index_t h = j1 - j0;
  if (w <= MIN_BLOCK_SIZE && h <= MIN_BLOCK_SIZE) {
    compute_block(iter_counts, width, i0, j0, i1, j1, xmin, ymin, dx, dy);
    return;
  }
...
#pragma omp task shared(iter_counts)
  compute_adaptive(iter_counts, width, i0, j0, im, jm, xmin, ymin, dx, dy);

#pragma omp task shared(iter_counts)
  compute_adaptive(iter_counts, width, im, j0, i1, jm, xmin, ymin, dx, dy);

#pragma omp task shared(iter_counts)
  compute_adaptive(iter_counts, width, i0, jm, im, j1, xmin, ymin, dx, dy);

#pragma omp task shared(iter_counts)
  compute_adaptive(iter_counts, width, im, jm, i1, j1, xmin, ymin, dx, dy);
}
...
#pragma omp single
  {
    compute_adaptive(iter_counts.data(), width, 0, 0, width, height, xmin, ymin,
                     dx, dy);
