...
program multiple_p2p_thread_comms
  use mpi
  use omp_lib
  implicit none
  ...
  if (rank == 0) then
    msg = tid
    do i = 1, ntasks - 1
      call MPI_Send(msg, 1, MPI_INTEGER, i, tag, mpi_comm_thread(tid + 1), ierr)
    end do
  else
    call MPI_Recv(msg, 1, MPI_INTEGER, 0, tag, mpi_comm_thread(tid + 1), MPI_STATUS_IGNORE, ierr)
...
end program multiple_p2p_thread_comms
